...
export default {
    ...
    actions: {
        ...
        async getJoinRequests({ commit }, clubID) {
            await axios.get('http://127.0.0.1:8000/api/request/joinclubrequest').then(res => {
                var joinArr = res.data.filter((req) => {
                    return req.club_id == clubID
                })
                commit('setJoinRequests', joinArr)
            }).catch(err => {
                console.log(err)
            })
        },
